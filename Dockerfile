...
ARG GO_MODULE="github.com/stuttgart-things/sweatShop-informer"
ARG VERSION=""
ARG BUILD_DATE=""
ARG COMMIT=""
...
RUN go mod tidy
RUN CGO_ENABLED=0 go build -o /bin/sweatShop-informer \
...
COPY --from=builder /bin/sweatShop-informer /bin/sweatShop-informer

ENTRYPOINT ["sweatShop-informer"]
